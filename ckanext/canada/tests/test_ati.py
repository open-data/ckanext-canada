...
filter_ati_path = os.path.join(os.path.dirname(str(__file__)), '../../../bin/filter/filter_ati.py')
spec = util.spec_from_file_location("canada.bin.filters.ati", filter_ati_path)
filter_ati = util.module_from_spec(spec)
sys.modules["canada.bin.filters.ati"] = filter_ati
spec.loader.exec_module(filter_ati)
...
class TestAti(CanadaTestBase):
    ...
    def test_example(self):
        ...
        """
        record = get_chromo('ati')['examples']['record'].copy()
    ...
    def test_primary_key_commas(self):
        ...
        """
        record = get_chromo('ati')['examples']['record'].copy()
        record['request_number'] = 'this,is,a,failure'
        ...
        assert 'request_number' in err['records'][0]
        assert err['records'][0]['request_number'] == ['Comma is not allowed in Request Number field']
    ...
    def test_required_fields(self):
        ...
        expected_required_fields = ['year', 'month',
    ...
    def test_choice_fields(self):
        ...
        expected_choice_fields = ['disposition',]
    ...
    def test_date_constraints(self):
        ...
        assert err['records'][0]['year'] == ['Please enter a valid year']
    ...
    def test_negative_page_num(self):
        """
        Cannot have pages be a negative number
        """
        record = get_chromo('ati')['examples']['record'].copy()
        ...
        err = ve.value.error_dict
        assert 'records' in err
        assert 'pages' in err['records'][0]
        assert err['records'][0]['pages'] == ['This value must not be negative']

    def test_filter_script(self):
        """
        Filter out any data pre 2020.
        ...
        test_record = filter_ati.test(dict(record))
...
class TestAtiNil(CanadaTestBase):
    ...
    def test_example(self):
        ...
        Example data should load
        """
        record = get_chromo('ati-nil')['examples']['record'].copy()
        self.lc.action.datastore_upsert(
            resource_id=self.resource_id,
            records=[record])
