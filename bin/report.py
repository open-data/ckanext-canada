...
cli()
'''
stdout.write('period,org')
for c in COLUMNS:
    stdout.write(',{0}'.format(c.split(',')[0]))
stdout.write('\n')

for tfn in sorted(glob('pd-*.tar.gz')):
    tf = tarfile.open(tfn)
    dt = tfn.split('pd-')[1].split('.tar.gz')[0]
    dt = dt[:4] + '-' + dt[4:6] + '-' + dt[6:]
    counts = {c:Counter() for c in COLUMNS}
    orgs = set()
    for cfn in tf.getnames():
        stderr.write('-- processing {0}: {1}\n'.format(tfn, cfn))
        if cfn not in COLUMNS:
            stderr.write('skipping\n')
            continue
        fcounts = counts[cfn]
        cf = tf.extractfile(cfn)
        records = DictReader(cf)
        for r in records:
            fcounts[r['owner_org']] += 1
        orgs = orgs.union(fcounts)

    for org in orgs:
        stdout.write('{0},{1}'.format(dt, org))
        for c in COLUMNS:
            stdout.write(',{0:d}'.format(counts[c][org]))
        stdout.write('\n')
    stdout.write('{0},COUNT'.format(dt))
    for c in COLUMNS:
        stdout.write(',{0:d}'.format(len(counts[c])))
    stdout.write('\n{0},SUM'.format(dt))
    for c in COLUMNS:
        stdout.write(',{0:d}'.format(sum(counts[c].values())))
    stdout.write('\n{0},MAX'.format(dt))
    for c in COLUMNS:
        stdout.write(',{0:d}'.format(max([0] + counts[c].values())))
    stdout.write('\n')
'''
