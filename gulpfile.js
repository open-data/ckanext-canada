...
function minifyCustomJS(){
  return src('ckanext/canada/assets/custom/source/*.js', {sourcemaps: false})
    .pipe(terser({'compress': true,
                  'mangle': true}))
    .pipe(rename(function(_fileObj){
      _fileObj.extname = '.min.js';
    }))
    .pipe(dest('ckanext/canada/assets/custom/build', {sourcemaps: false}));
}

function minifyDatatablesJS(){
  return src('ckanext/canada/assets/datatables/source/*.js', {sourcemaps: false})
    .pipe(terser({'compress': true,
                  'mangle': true}))
    .pipe(rename(function(_fileObj){
      _fileObj.extname = '.min.js';
    }))
    .pipe(dest('ckanext/canada/assets/datatables/build', {sourcemaps: false}));
}

function minifyIMJS(){
  return src('ckanext/canada/assets/invitation-manager/vendor/*.js', {sourcemaps: false})
    .pipe(terser({'compress': true,
                  'mangle': false}))
    .pipe(rename(function(_fileObj){
      _fileObj.extname = '.min.js';
